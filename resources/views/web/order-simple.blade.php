            ...
            <form id="orderForm" method="POST" action="{{ route('web.orders.store') }}">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12">
                        ...
                        <input type="text" class="cs_shop-input" name="cardholder_name" required>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showPaymentModal() {
        ...
        const total = parseFloat(document.getElementById('orderTotal').textContent.replace('$', '')) || 0;
    ...
    function hidePaymentModal() {
        ...
        if (payNowBtnText) payNowBtnText.innerHTML = '<i class="fas fa-lock" style="margin-right: 8px;"></i>Complete Payment';
    }
    ...
    function processPayment() {
        ...
        console.log('Step 2: Validating card number');
        
        if (!cardNumber || cardNumber.length < 13 || cardNumber.length > 19) {
            console.error('Step 2: Invalid card number');
        ...
        console.log('Step 3: Validating card expiry');
        
        if (!cardExpiry || !/^\d{2}\/\d{2}$/.test(cardExpiry)) {
            console.error('Step 3: Invalid expiry date');
        ...
        console.log('Step 4: Validating CVC');
        
        if (!cardCvc || cardCvc.length < 3 || cardCvc.length > 4) {
            console.error('Step 4: Invalid CVC');
        ...
        const total = parseFloat(document.getElementById('orderTotal').textContent.replace('$', '')) || 0;
        console.log('Step 5: Cart total validated:', total);
        
        if (total <= 0) {
            console.error('Step 5: Invalid total amount');
        ...
        console.log('Step 6: Customer email:', email);
        
        if (!email) {
            console.error('Step 6: Email is required');
            showPaymentError('Email is required');
            return;
        }

        console.log('Step 7: All validations passed, preparing form submission');
        
        // Hide modal and submit form with payment data
        hidePaymentModal();
        
        const orderForm = document.getElementById('orderForm');
        if (orderForm) {
            console.log('Step 8: Adding payment fields to form');
            
            // Add cardholder name to the existing field
            const existingCardholderField = orderForm.querySelector('[name="cardholder_name"]');
            if (existingCardholderField) {
                existingCardholderField.value = cardholderName;
            }

            // Add hidden card fields for backend processing
            const cardFields = [
                {name: 'card_number', value: cardNumber},
                {name: 'card_expiry', value: cardExpiry},
                {name: 'card_cvc', value: cardCvc}
            ];

            cardFields.forEach(field => {
                let hiddenInput = orderForm.querySelector(`[name="${field.name}"]`);
                if (!hiddenInput) {
            ...
            console.log('Step 9: Submitting form to backend for complete processing');
            
            // Show loading message
            Swal.fire({
                title: 'Processing Payment...',
                text: 'Please wait while we process your payment and create your order.',
                icon: 'info',
            ...
            // Submit the form - everything will be handled on the backend
            orderForm.submit();
    ...
    // Prevent direct form submission - show payment modal instead
        ...
        <form id="paymentForm">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input 
                    ...
                    maxlength="19"
                    required 
                    style="font-size: 16px; padding: 14px; border: 2px solid #ddd; border-radius: 8px; margin-bottom: 10px;"
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                    ...
                    <input 
                        ...
                        maxlength="5"
                        required 
                        style="font-size: 16px; padding: 14px; border: 2px solid #ddd; border-radius: 8px; width: 50%;"
                        autocomplete="cc-exp"
                    >
